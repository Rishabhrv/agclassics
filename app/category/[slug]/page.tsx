    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2 pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={[
            "ac-toast max-w-[280px] px-[18px] py-[11px] text-[11px] tracking-[1.5px] uppercase backdrop-blur-md border",
...
/* ══════════════════════════════════════════════════════
   ...
   - inCart         — whether this product is already in the user's cart
   - onCartAdded    — called after a successful add so the parent can update its set
══════════════════════════════════════════════════════ */
function ProductCard({
  ...
  // Out-of-stock only applies to physical / both; ebooks are never OOS
  ...
  const addToCart = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Surface the actual backend message (e.g. "Out of stock")
        showToast(data?.message || "Could not add to cart", "error"); return;
      }

      window.dispatchEvent(new Event("cart-change"));
      showToast(`"${product.title}" added to cart`, "success");
      onCartAdded(product.id);           // ← tell parent to mark as in-cart
  ...
  /* ── derive cart button appearance ── */
  const cartDisabled = isOos || loadingCart || inCart;

  const cartBtnBg = isOos
    ? "#2a2a2d"
    : inCart
    ? "rgba(201,168,76,.12)"
    : "#c9a84c";
  ...
  const cartBtnHoverBg = inCart || isOos ? undefined : "#f5f0e8";
  ...
  return (
    <>
      ...
      >
        ...
        <div className="relative overflow-hidden" style={{ aspectRatio: "3/4" }}>
          ...
          <div className="absolute top-2.5 left-2.5 z-[2] flex flex-col gap-[5px]">
            {isOos ? (
              ...
              <span className="text-[8px] tracking-[1.5px] uppercase px-2 py-0.5 bg-[#c9a84c] text-[#0a0a0b]"
                style={{ fontFamily: "'Jost', sans-serif" }}>New</span>
            ) : null}

            {/* Format pill — only shown on "ebook" or "both" */}
            {product.product_type !== "physical" && (
              <span
                className="text-[7px] tracking-[1.5px] uppercase px-2 py-0.5"
                style={{
                  fontFamily: "'Jost', sans-serif",
                  background: product.product_type === "ebook"
                    ? "rgba(201,168,76,.18)"
                    : "rgba(255,255,255,.08)",
                  color: product.product_type === "ebook" ? "#c9a84c" : "#a0a0a0",
                  border: product.product_type === "ebook"
          ...
          <div
            className="absolute inset-0 flex flex-col justify-end px-3.5 pb-3.5 transition-all duration-[350ms]"
            style={{ background: "linear-gradient(to top, rgba(10,10,11,.96) 0%, rgba(10,10,11,.45) 45%, transparent 70%)" }}
          >
            {/* Slide-up action row */}
            <div className="flex gap-1.5 opacity-0 translate-y-[10px] transition-[opacity,transform] duration-[350ms] group-hover:opacity-100 group-hover:translate-y-0">
              ...
              <button
                ...
                onClick={addToCart}
                className={[
                  "flex-1 text-[9px] tracking-[2px] uppercase font-medium py-[9px] px-1.5 border-none",
                  "flex items-center justify-center gap-[5px] transition-colors duration-[250ms]",
                  cartDisabled && !inCart
                    ? "cursor-not-allowed"
                    : inCart
                    ? "cursor-default"
              ...
              >
                {loadingCart ? (
                  <span className="w-[10px] h-[10px] rounded-full animate-spin border-[1.5px] border-[rgba(10,10,11,.3)] border-t-[#0a0a0b]" />
                ) : inCart ? (
                  <>
              ...
              <button
                ...
                className={[
                  "w-9 shrink-0 flex items-center justify-center border",
        ...
        {/* ── Text block ── */}
        <div className="px-4 pt-3.5 pb-[18px] border-t border-[rgba(201,168,76,.07)]">
          <h3
            className="text-[16px] font-semibold text-[#f5f0e8] leading-[1.25] mb-2 line-clamp-2"
            style={{ fontFamily: "'Cormorant Garamond', serif" }}
          >
            {product.title}
          </h3>
          {product.rating > 0 && <div className="mb-2"><Stars rating={product.rating} /></div>}
...
export default function CategoryPage() {
  ...
  /**
   * Set of product IDs already present in the user's cart.
   * Populated on mount and kept up-to-date via optimistic updates.
   */
  const [cartProductIds, setCartProductIds] = useState<Set<number>>(new Set());

  const totalPages = Math.ceil(total / LIMIT);

  /* ── Fetch cart IDs so we can show "In Cart" state on load ── */
  ...
  useEffect(() => {
    fetchCartIds();
    // Re-sync whenever the cart changes (e.g. header badge update)
    window.addEventListener("cart-change", fetchCartIds);
    return () => window.removeEventListener("cart-change", fetchCartIds);
  }, [fetchCartIds]);

  /** Called by ProductCard after a successful add-to-cart — no need to re-fetch */
...
  }, [slug]);
  ...
  const activeFilters =
    selectedRating > 0 || !!selectedAuthor || priceInput[0] > 0 || priceInput[1] < 5000;
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0a0a0b] pt-[140px] md:pt-[120px]">
        ...
        <div className="max-w-[1400px] mx-auto px-25 py-8 grid grid-cols-[240px_1fr] gap-10 items-start
          ...
          {/* Desktop sidebar */}
          <aside className="sticky top-[100px] max-[900px]:hidden">
          ...
          <main className="ml-15">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
              ...
              <button
                ...
                className="hidden max-[900px]:flex items-center gap-2
                  bg-[rgba(201,168,76,.08)] border border-[rgba(201,168,76,.2)]
                  px-4 py-2 text-[#c9a84c] text-[10px] tracking-[2px] uppercase cursor-pointer"
                style={{ fontFamily: "'Jost', sans-serif" }}
              >
              ...
              {/* Search */}
              <div className="flex items-center gap-2 bg-[rgba(201,168,76,.04)] border border-[rgba(201,168,76,.1)] px-3.5 py-2 flex-1 max-w-[300px]">
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                <div className="w-[60px] h-[60px] mx-auto mb-5 flex items-center justify-center border border-[rgba(201,168,76,.15)]">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#6b6b70" strokeWidth="1">
        ...
        {sidebarOpen && (
          <>
            ...
            <div className="fixed top-0 left-0 bottom-0 w-[300px] z-[201] bg-[#141415]
              border-r border-[rgba(201,168,76,.12)] overflow-y-auto p-6">
      ...
      <div className={sectionCls}>
        ...
        {[4, 3, 2, 1].map((r) => (
          <button key={r}
            ...
            className={["flex items-center gap-2.5 w-full px-2 py-[7px] mb-1 cursor-pointer border bg-transparent transition-all duration-200",
          ...
          {authors.map((author) => {
            ...
            return (
              <div key={author.id}
                ...
                className={["flex items-center gap-2.5 px-2.5 py-2 cursor-pointer border transition-colors duration-200",
...
function FilterTag({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <div className="flex items-center gap-2 bg-[rgba(201,168,76,.08)] border border-[rgba(201,168,76,.2)] pl-3 pr-2.5 py-1">
